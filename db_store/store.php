...
class Users {
	...
	function __construct(){
		...
		$dbName = 'vision_fb_db'; //Define database name
	...
	function checkUser($oauth_provider,$oauth_uid,$fname,$lname,$email,$gender,$locale,$picture){
		...
		$prev_query = mysqli_query($this->connect,"SELECT * FROM ".$this->table_name." WHERE email = '".$email."' AND oauth_uid = '".$oauth_uid."'") or die(mysql_error($this->connect));
		if(mysqli_num_rows($prev_query)>0){
			$update = mysqli_query($this->connect,"UPDATE $this->table_name SET oauth_provider = '".$oauth_provider."', oauth_uid = '".$oauth_uid."', fname = '".$fname."', lname = '".$lname."', email = '".$email."', gender = '".$gender."', locale = '".$locale."', picture = '".$picture."', modified = '".date("Y-m-d H:i:s")."' WHERE email = '".$email."' AND oauth_uid = '".$oauth_uid."'");
		}else{
